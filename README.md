...
- [功能特性](#功能特性)
- [与 Data Agent 联动](#与-data-agent-联动)
- [快速开始](#快速开始)
- [使用指南](#使用指南)
- [AI 模型配置](#ai-模型配置)
- [数据库连接](#数据库连接)
...
SQL Prompt Copilot 可以与 [Data Analysis Agents](https://github.com/echo1229/Data_analysis_agents) 配合使用，实现 **SQL 生成 → 数据分析 → 飞书报告** 的完整闭环。

```
┌─────────────────────┐         ┌─────────────────────┐
│  SQL Prompt Copilot │         │   Data Analysis      │
│  (桌面端 SQL 生成)    │         │   Agents (飞书 Bot)   │
├─────────────────────┤         ├─────────────────────┤
│ 1. 用户输入自然语言   │         │                     │
│ 2. BM25 匹配 Schema  │         │                     │
│ 3. AI 生成 SQL       │────────▶│ 4. 接收 SQL + 提示词  │
│ 4. DAG 可视化审查    │ 一键发送 │ 5. Critic 深度审查    │
│ 5. 用户确认无误      │         │ 6. 执行 SQL 查询      │
│                     │         │ 7. 生成业务分析报告    │
│                     │         │ 8. 发送飞书卡片消息    │
└─────────────────────┘         └─────────────────────┘
```
...
1. 在 Copilot 中输入问题，生成 SQL 和 DAG 图
2. 审查 SQL 逻辑和执行流程，确认无误
3. 点击 **"发送至飞书"** 按钮
4. 在弹出的对话框中编辑提示词（可选）
5. 点击发送，Data Agent 会自动执行分析并发送报告到飞书

**前置条件：**
- Data Agent 服务已启动（`python feishu_server.py`）
- ngrok 隧道已开启（用于飞书 Webhook）
- Copilot 中已配置数据库连接（Schema 会自动传递）

**纠错知识库：**

Copilot 内置纠错知识库，记录历史 SQL 错误和修正方案：
- 点击 **"纠错"** 按钮查看和管理纠错记录
- 生成 SQL 时自动匹配相似问题的历史纠错
- 纠错信息会注入到提示词中，提高生成准确率
